...
private extension SearchViewModel {
    ...
    func runPurchaseCheck(for app: IpatoolApp, environment: CommandEnvironment) async {
        ...
        // If all commands failed, schedule a retry after delay
        let retryDelaySeconds: UInt64 = 2_000_000_000 // 2 seconds
        Task { [weak self] in
            try? await Task.sleep(nanoseconds: retryDelaySeconds)
            await self?.runPurchaseCheck(for: app, environment: environment)
        }
    }
...
actor AsyncSemaphore {
    ...
    func wait() async {
        ...
        // After resuming from wait, we now have the permit
        current += 1
    }
